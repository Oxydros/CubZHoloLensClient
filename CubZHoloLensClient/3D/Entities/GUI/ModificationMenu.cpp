...
void HoloLensClient::ModificationMenu::InitializeMenu()
{
	...
	auto _button1 = std::make_unique<Button>(_devicesResources, _scene, _callbackRotateL, float2(0.045f, 0.07f));
	_button1->SetRelativePosition({ -0.08f, 0.0f, 0.01f });
	_button1->setLabel(L"RotL");
	_button1->disableMultipleClick(false);

	auto _button3 = std::make_unique<Button>(_devicesResources, _scene, _callbackRotateR, float2(0.045f, 0.07f));
	_button3->SetRelativePosition({ -0.035f, 0.0f, 0.01f });
	_button3->setLabel(L"RotR");
	_button3->disableMultipleClick(false);

	auto _button2 = std::make_unique<Button>(_devicesResources, _scene, _callbackMove, float2(0.09f, 0.07f));
...
void HoloLensClient::ModificationMenu::AttachEntity(EditableEntity *entity)
{
	...
	_callbackMove = std::bind(&EditableEntity::OnMoveClick, _attachedEntity);
	_callbackRotateL = std::bind(&EditableEntity::OnRotateLeftClick, _attachedEntity);
	_callbackRotateR = std::bind(&EditableEntity::OnRotateRightClick, _attachedEntity);
	setVisible(true);
}
...
void HoloLensClient::ModificationMenu::DetachEntity()
{
	...
	_callbackMove = nullptr;
	_callbackRotateL = nullptr;
	_callbackRotateR = nullptr;
	setVisible(false);
}
