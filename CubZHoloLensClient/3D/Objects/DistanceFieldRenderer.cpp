...
DistanceFieldRenderer::DistanceFieldRenderer(const std::shared_ptr<DX::DeviceResources>& deviceResources,
											 unsigned int const& textureWidth, unsigned int const& textureHeight)
...
void DistanceFieldRenderer::RenderDistanceField(ID3D11ShaderResourceView* texture)
{
	...
	//ID3D11RenderTargetView *saveRenderTarget;
	//ID3D11DepthStencilView *savestencilView;

	//context->OMGetRenderTargets(1, &saveRenderTarget, &savestencilView);

	//D3D11_VIEWPORT saveViewport;
	//UINT numViewport = 1;
	//context->RSGetViewports(&numViewport, &saveViewport);
	...
	//context->OMSetRenderTargets(1, &saveRenderTarget, savestencilView);
	//context->RSSetViewports(1, &saveViewport);
}
